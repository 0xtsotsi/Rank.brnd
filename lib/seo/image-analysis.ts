...
function extractImages(html: string): Array<{
  ...
  const images: Array<{ src: string; alt: string; hasAlt: boolean; altIsDescriptive: boolean }> = [];
  let match;
  ...
  while ((match = imgRegex.exec(html)) !== null) {
    ...
    const altIsDescriptive = hasAlt && alt.length > 8 &&
      !/^(image|photo|picture|img|pic|icon|logo|placeholder)$/i.test(alt.trim());
...
export function analyzeImages(content: string): ImageAnalysis {
  ...
  const imagesWithDescriptiveAlt = images.filter(img => img.altIsDescriptive).length;
  ...
  if (totalImages === 0) {
    ...
    const descriptivePercentage = images.length > 0
      ? (imagesWithDescriptiveAlt / totalImages) * 100
      : 0;
...
export function calculateImageAltScore(analysis: ImageAnalysis): number {
  ...
  const descriptiveScore = analysis.totalImages > 0
...
export function getImageRecommendations(analysis: ImageAnalysis): string[] {
  ...
  if (analysis.totalImages === 0) {
    recommendations.push('Consider adding relevant images to make your content more engaging.');
  ...
  const withButNotDescriptive = analysis.imagesWithAlt - analysis.imagesWithDescriptiveAlt;
  ...
  if (analysis.imagesWithDescriptiveAlt === analysis.totalImages && analysis.totalImages > 0) {
