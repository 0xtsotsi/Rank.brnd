...
const outlineSectionSchema = z.object({
  ...
  title: z.string().min(1, 'Section title is required').max(500, 'Section title too long'),
  points: z.array(z.string().min(1, 'Bullet point cannot be empty')).default([]),
  wordCount: z.number().int().nonnegative('Word count must be non-negative').default(200),
});

/**
 * Brand voice configuration schema
 */
const brandVoiceConfigSchema = z.object({
  tone: z.array(z.string()).optional(),
  formality_level: z.enum(['formal', 'informal', 'neutral']).optional(),
  vocabulary: z.object({
    category: z.string().optional(),
    complexity_level: z.enum(['simple', 'moderate', 'complex', 'academic']).optional(),
    common_words: z.array(z.string()).optional(),
  }).optional(),
  style: z.object({
    sentence_structure: z.enum(['simple', 'compound', 'complex', 'varied']).optional(),
    paragraph_length: z.enum(['short', 'medium', 'long', 'varied']).optional(),
...
const seoConfigSchema = z.object({
  ...
  meta_description_length: z.number().int().positive().optional().default(160),
...
export const generateArticleDraftSchema = z.object({
  ...
  outline: z.array(outlineSectionSchema).min(1, 'At least one outline section is required'),
  ...
  custom_instructions: z.string().max(5000, 'Custom instructions too long').optional(),
...
export const listArticleDraftsQuerySchema = z.object({
  ...
  sort: z.enum(['created_at', 'updated_at', 'word_count', 'keyword']).optional().default('created_at'),
...
export const updateArticleDraftSchema = z.object({
  id: z.string().min(1, 'Draft ID is required'),
  organization_id: z.string().min(1, 'Organization ID is required'),
  content_updates: z.object({
    title: z.string().min(1).max(500).optional(),
    content: z.string().optional(),
    excerpt: z.string().optional(),
    add_internal_link: z.object({
      anchor_text: z.string().min(1),
      target_keyword: z.string().min(1),
      section_id: z.string().optional(),
    }).optional(),
    remove_internal_link: z.object({
      anchor_text: z.string().min(1),
    }).optional(),
  }).optional(),
  regen_metadata: z.object({
