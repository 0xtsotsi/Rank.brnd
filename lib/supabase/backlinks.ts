...
export async function getOrganizationBacklinks(
  ...
  options: {
    ...
    limit?: number;
    offset?: number;
    sortBy?: 'source_url' | 'target_url' | 'domain_authority' | 'page_authority' | 'status' | 'first_seen_at' | 'last_verified_at' | 'created_at';
    sortOrder?: 'asc' | 'desc';
  } = {}
): Promise<BacklinkResult<Backlink[]>> {
  try {
    ...
    if (options.search) {
      query = query.or(`source_url.ilike.%${options.search}%,target_url.ilike.%${options.search}%,anchor_text.ilike.%${options.search}%`);
    }
...
export async function getProductBacklinks(
  ...
  try {
    ...
    query = query.order('domain_authority', { ascending: false, nullsFirst: false });
  ...
  try {
    const { data, error } = await client
      ...
      .insert({
        ...
        tags: (backlink.tags || DEFAULT_BACKLINK_VALUES.tags) as unknown as Json,
        metadata: (backlink.metadata || DEFAULT_BACKLINK_VALUES.metadata) as unknown as Json,
      })
      .select()
      .single();
...
export async function bulkCreateBacklinks(
...
): Promise<BacklinkResult<{ successful: number; failed: number; errors: string[] }>> {
  const errors: string[] = [];
  let successful = 0;
  ...
  for (const bl of backlinks) {
    try {
      ...
      errors.push(`${bl.source_url}: ${error instanceof Error ? error.message : 'Unknown error'}`);
    }
  }
  ...
  try {
    ...
    return {
      ...
      data: {
        ...
        byStatus,
        byLinkType,
        avgDomainAuthority: domainAuthorityCount > 0 ? totalDomainAuthority / domainAuthorityCount : 0,
    ...
    return {
      success: false,
      error:
        error instanceof Error ? error.message : 'Failed to fetch backlink stats',
    };
  }
}
...
export function validateBacklink(backlink: {
  ...
  if (backlink.domain_authority !== undefined && (typeof backlink.domain_authority !== 'number' || backlink.domain_authority < 0 || backlink.domain_authority > 100)) {
    errors.push('Domain authority must be a number between 0 and 100');
  }

  if (backlink.page_authority !== undefined && (typeof backlink.page_authority !== 'number' || backlink.page_authority < 0 || backlink.page_authority > 100)) {
    errors.push('Page authority must be a number between 0 and 100');
  }

  if (backlink.spam_score !== undefined && (typeof backlink.spam_score !== 'number' || backlink.spam_score < 0 || backlink.spam_score > 1)) {
