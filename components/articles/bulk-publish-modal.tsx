...
import { PUBLISHING_PLATFORM_LABELS, PUBLISHING_PLATFORM_COLORS } from '@/types/publishing-queue';
...
export function BulkPublishModal({
  ...
  const [publishProgress, setPublishProgress] = useState({ current: 0, total: articles.length });
  ...
  const handlePublish = useCallback(async () => {
    ...
    try {
      ...
      setError(err instanceof Error ? err.message : 'Failed to publish articles');
      setStep('select');
    } finally {
      setIsPublishing(false);
    }
  }, [articles, selectedIntegration, selectedPlatform, scheduledFor, priority, onSuccess]);
              ...
              {activeIntegrations.length > 0 ? (
                <div className="grid grid-cols-2 gap-3">
                  {activeIntegrations.map((integration) => {
                    const platformInfo = PUBLISHING_PLATFORM_COLORS[integration.platform as PublishingPlatform] || {
                      bg: 'bg-gray-100 dark:bg-gray-700',
                      text: 'text-gray-700 dark:text-gray-300',
                      icon: integration.platform.charAt(0).toUpperCase(),
                    };
                        ...
                        <div className="flex items-center gap-2">
                          ...
                          <div className="flex-1 min-w-0">
                            ...
                            <p className="text-xs text-gray-500 dark:text-gray-400">
                              {PUBLISHING_PLATFORM_LABELS[integration.platform as PublishingPlatform] || integration.platform}
                            </p>
                          </div>
                          {isSelected && (
                  ...
                  <div className="flex items-start gap-3">
                    ...
                    <div>
                      ...
                      <p className="text-sm text-yellow-700 dark:text-yellow-400 mt-1">
                        Please connect a CMS platform integration first to publish articles.
                      </p>
                    </div>
                  </div>
                </div>
            ...
            <div>
              ...
              <div className="flex flex-wrap gap-2">
                {[
                  ...
                  { value: 5, label: 'Normal', color: 'bg-blue-100 text-blue-700 dark:bg-blue-900/30 dark:text-blue-300' },
                  { value: 10, label: 'High', color: 'bg-amber-100 text-amber-700 dark:bg-amber-900/30 dark:text-amber-300' },
                ].map((p) => (
                  <button
                    key={p.value}
                    type="button"
                    ...
                    className={cn(
                      ...
                      priority === p.value
                        ? 'ring-2 ring-indigo-500 ring-offset-2 dark:ring-offset-gray-800 ' + p.color
                        : p.color
                    )}
                  >
                    {p.label}
        ...
        return (
          <div className="text-center py-8">
            ...
            <p className="text-gray-600 dark:text-gray-400 mb-6">
              {scheduledFor ? 'Scheduling articles for publishing' : 'Publishing articles to CMS'}
            </p>
      ...
      case 'complete':
        return (
          <div className="text-center py-8">
            ...
            <h3 className="text-xl font-semibold text-gray-900 dark:text-white mb-2">
              {scheduledFor ? 'Articles Scheduled!' : 'Articles Queued for Publishing!'}
            </h3>
            <p className="text-gray-600 dark:text-gray-400 mb-6">
              {publishResult?.message || `${articles.length} articles have been added to the publishing queue.`}
            </p>
            {publishResult && (
              <div className="max-w-md mx-auto grid grid-cols-2 gap-4">
                <div className="p-4 bg-green-50 dark:bg-green-900/20 rounded-lg">
                  ...
                  <p className="text-sm text-green-600 dark:text-green-500">Successful</p>
                </div>
  ...
  return (
    <Modal
      isOpen={isOpen}
      onClose={handleClose}
      title={step === 'publishing' ? '' : step === 'complete' ? '' : 'Bulk Publish Articles'}
      size="lg"
        ...
        step === 'select' || step === 'schedule' ? (
          <div className="flex items-center justify-between w-full">
            ...
            <Button
              ...
              disabled={!selectedIntegration || articles.length === 0 || isPublishing}
