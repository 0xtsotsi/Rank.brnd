...
export class MockClerkAuth {
  ...
  /**
   * Setup authenticated request context
   * This adds the necessary headers to simulate an authenticated request
   */
  async setupAuthenticatedRequest(
    request: APIRequestContext,
    user: Partial<TestUser> = {}
  ): Promise<AuthTokens> {
    const tokens = this.generateAuthTokens(user);

    // Store tokens for use in requests
    return tokens;
  }
}
...
export class OrganizationMembershipFactory {
  /**
   * Create a test organization membership record
   */
  static createMembership(overrides: {
    userId?: string;
    ...
    return {
      ...
      user_id: overrides.userId || `user_${Date.now()}`,
      organization_id: overrides.organizationId || `org_${Date.now()}`,
  ...
  /**
   * Create multiple memberships for testing RBAC
   */
  static createRbacMemberships(orgId: string): Array<{
    userId: string;
    role: 'owner' | 'admin' | 'member' | 'viewer';
    membership: any;
  }> {
